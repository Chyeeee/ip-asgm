...
# IMPORTANT:
# Fruit classifier uses colour + texture + shape information.
# Damage and ripeness/quality labels are excluded.
# The seven shape_* columns are intentionally retained.
...
print(
    f"\nFinal colour + texture + shape features: "
    f"{X.shape[1]}"
)

print(
    f"Shape features included: "
    f"{len(shape_columns)}"
)
...
if len(shape_columns) != 7:
    raise ValueError(
        "Expected 7 shape features, but found "
        f"{len(shape_columns)}. Regenerate "
        "colour_texture_features.csv first."
    )
